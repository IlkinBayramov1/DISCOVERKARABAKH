...
export const ReviewList: React.FC<ReviewListProps> = ({ reviews, isLoading, onReport }) => {
    if (isLoading) {
        return <div className="loading-reviews">Discovery voices are arriving...</div>;
    }

    if (reviews.length === 0) {
        return <div className="no-reviews">No stories shared yet. Be the first to capture the moment!</div>;
    }
