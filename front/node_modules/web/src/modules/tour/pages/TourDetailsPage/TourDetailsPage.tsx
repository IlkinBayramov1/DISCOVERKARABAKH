                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <div className="lg:w-2/3">
                        ...
                        {tour.images && tour.images.length > 0 && (
                            <div className="mb-12">
                                <div className="rounded-2xl overflow-hidden shadow-xl aspect-video relative mb-4">
                                    <img src={tour.images[0]} alt={tour.name} className="w-full h-full object-cover" />
                                </div>
                                {tour.images.length > 1 && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {tour.images.slice(1).map((img, idx) => (
                                            <div key={idx} className="rounded-xl overflow-hidden shadow-md aspect-video relative">
                                                <img src={img} alt={`${tour.name} ${idx + 2}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" />
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    <div className="lg:w-1/3">
                        <div className="sticky top-24 bg-white p-8 rounded-3xl shadow-2xl border border-gray-100">
                            ...
                            <div className="mb-8">
                                ...
                                {tour.startDate ? (
                                    <div className="py-3 px-4 rounded-xl text-lg font-bold bg-blue-50 border-2 border-blue-200 text-blue-800 flex items-center justify-center shadow-inner">
                                    ...
                                    <div className="p-4 bg-gray-50 rounded-xl border border-dashed border-gray-200 text-center">
                                        <p className="text-gray-500 text-sm">Tour date has not been specified.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            {availability && (
                                <div className="mb-8 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-blue-800 font-semibold">Availability</span>
                                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${availability.isFull ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                                            }`}>
                                            {availability.isFull ? 'FULLY BOOKED' : 'AVAILABLE'}
                                        </span>
                                    </div>
                                    <div className="w-full bg-blue-200 h-2 rounded-full overflow-hidden">
                                        <div
                                            className="bg-blue-600 h-full transition-all duration-500"
                                            style={{ width: `${(availability.bookedCount / availability.maxSeats) * 100}%` }}
                                        />
                                    </div>
                                    <p className="text-xs text-blue-600 mt-2 font-medium">
                                        {availability.remainingSeats} seats remaining of {availability.maxSeats}
                                    </p>
                                </div>
                            )}

                            {availabilityLoading && !availability && (
                                <div className="mb-8 p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-pulse">
                            ...
                            <button
                                ...
                                disabled={availability?.isFull || availabilityLoading}
                                className={`w-full py-4 rounded-2xl font-black text-lg transition-all shadow-xl mb-6 ${availability?.isFull
                                        ? 'bg-gray-200 text-gray-400 cursor-not-allowed'
                                        : 'bg-blue-600 text-white hover:bg-blue-700 hover:scale-[1.02] shadow-blue-200'
                                    }`}
                            >
                                {availability?.isFull ? 'Sold Out' : 'Book This Adventure'}
                            </button>
